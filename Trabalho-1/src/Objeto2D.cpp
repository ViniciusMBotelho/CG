...
glm::vec2 Objeto2D::calcularCentroAtual() const {
    glm::vec2 cOrig = calcularCentroOriginal();
    return Transformacoes::aplicar(matrizAcumulada, cOrig);
}
...
void Objeto2D::cisalhar(float hx, float hy) {
    if (hx != 0.0f) {
        comporTransformacao(Transformacoes::cisalhamentoX(hx));
    }
    if (hy != 0.0f) {
        comporTransformacao(Transformacoes::cisalhamentoY(hy));
    }
}
